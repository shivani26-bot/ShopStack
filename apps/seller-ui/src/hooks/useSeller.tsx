...
const useSeller = () => {
  ...
  const {
    data: seller, //data → the actual fetched user. Renamed to user with data: user.
    ...
    queryKey: ["seller"], //Unique key for this query. React Query uses it to cache and identify this data.Example: all components using ["user"] will share the same cached data.
    queryFn: fetchSeller, //React Query calls this function automatically.Function that actually fetches the data (probably does an Axios GET request).
