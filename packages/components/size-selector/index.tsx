  ...
  return (
    <div className="mt-2">
      ...
      <Controller
        name="sizes"
        control={control}
        // If you just want to directly return JSX, wrap it with () instead of {}:
        // If you need to run some extra logic before returning, use {} and explicitly return:
        render={({ field }) => (
          <div className="flex gap-2 flex=wrap">
            {sizes.map((size) => {
              const isSelected = (field.value || []).includes(size); //returns true or false
              return (
                <button
                  type="button"
                  key={size}
